...
async def generate_otp(
    ...
    """
    logger.info("OTP generation request received", username=request.username, contact_no=request.contact_no)
    ...
    if request.username:
        ...
        try:
            ...
            logger.warning("OTP generation failed - user not found by username", username=request.username)
            raise 
...
async def verify_otp(
    ...
    """
    logger.info("OTP verification request received", contact_no=request.contact_no,username=request.username)
    users_company = []
    if request.username:
        try:
            user = await user_service.get_user_by_username(request.username)
            ...
            if user.is_super_admin:
                ...
                users_company.append(VerifyOTPResponse.Company(
                    company_id=user.company_id,
            ...
            return ResponseModel(
                ...
                data=VerifyOTPResponse(
                    token=token,
                    companies=users_company
                ),
            )
        except UserNotFoundException:
            logger.warning("OTP verification failed - user not found by username", username=request.username)
            raise UnauthorizedException("Invalid credentials")
    elif request.contact_no:
        users = await user_service.get_users_by_contact_no(request.contact_no)
        if not users:
            logger.warning("OTP verification failed - user not found by contact_no", contact_no=request.contact_no)
            raise UnauthorizedException("Invalid credentials")
        for user in users:
...
async def auth_user_to_company(
    ...
    user = await user_service.get_user_by_contact_no_and_company(contact_no, request.company_id)
    if not user:
        logger.warning("Authentication to company failed - user not associated with company", company_id=str(request.company_id))
    ...
    logger.info("User authenticated to company successfully", user_id=str(user.id), company_id=str(request.company_id))

"""
Shared database table schema definitions for the agentic system.

This dictionary describes every tenant-schema table so that LLM agents
can understand the data model, generate correct SQL, and call the right
API tools with the right parameters.
"""

TABLE_SCHEMA_DICT: dict[str, dict] = {
    "salesforce.users": {
    ...
    "distributor_routes": {
        ...
        "indexes": [
            "idx_distributor_routes_distributor_id ON distributor_id",
            "idx_distributor_routes_route_id ON route_id",
        ],
    },
    "brand": {
    ...
    "products": {
        ...
        "indexes": [
            ...
            "idx_products_brand_category_id ON brand_category_id",
            "idx_products_brand_subcategory_id ON brand_subcategory_id",
        ],
    },
    "product_prices": {
...
def get_table_schemas(table_names: list[str]) -> dict[str, dict | str]:
    """Return schema definitions for the requested table names."""
    result: dict[str, dict | str] = {}
    for name in table_names:
        schema = TABLE_SCHEMA_DICT.get(name)
        result[name] = schema if schema is not None else "Schema not found"
    return result


def get_all_table_names() -> list[str]:
    """Return all available table names."""
    return list(TABLE_SCHEMA_DICT.keys())
